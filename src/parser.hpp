...
namespace {
    const std::unordered_map<std::string, eFlatSatCommands> cmdMap = {
        ...
        {"register", CMD_REGISTER_SENSOR},
        {"unregister", CMD_UNREGISTER_SENSOR},
...
class CParse {
  public:
    static SParsedArgs parsePacket(void* packet, size_t size) {
        ...
        switch (cmd->second) {
            case CMD_TRACK_SENSOR: pkt.sensor = args[0]; break;
            case CMD_UNTRACK_SENSOR: pkt.sensor = args[0]; break;
            case CMD_REGISTER_SENSOR: pkt.sensor = args[0]; break;
            case CMD_UNREGISTER_SENSOR: pkt.sensor = args[0]; break;
            case CMD_SET_MEASUREMENT_PERIOD:
                meas = strToU64(args[0].c_str());
                if (meas == 0U) {
                    logs::log(ERR, "Request argument was not valid! Provided Argument: %s\n", args[0].c_str());
                    SParsedArgs err = {
                        .error = "Argument was not valid!! It should be a unsigned integer. Provided: " + args[0],
                    };
                    return err;
                }
                pkt.measPeriod = meas;
